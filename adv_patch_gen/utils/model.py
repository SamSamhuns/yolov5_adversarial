...
def unwrap_preds(output):
    ...
    yolov5 in eval mode returns (inference_out, per_layer_out) and DetectMultiBackend hands that back as a list, while
    single output backends (onnx, tflite, ...) return a bare tensor. Indexing [0] unconditionally would slice the batch
    dimension for the latter.
    """
...
class UltralyticsBackend(torch.nn.Module):
    ...
    The head emits [batch, 4 + n_classes, n_preds] with no objectness channel, which MaxProbExtractor detects and
    handles.
    """
...
def load_detector(weights: str, device: torch.device, backend: str | None = None) -> torch.nn.Module:
    """Load the detector the patch is optimized against.

    Args:
        weights: path to the detector weights
        device: torch device to place the model on
        backend: "yolov5" for this repo's DetectMultiBackend (all its export formats), "ultralytics" for a v8/v11
            checkpoint loaded through the ultralytics package. None means yolov5.
        Note: only torch backends can be used for patch *training*. Every exported format in DetectMultiBackend returns
            predictions via torch.from_numpy, which severs the autograd graph, so no gradient can reach the patch. They
            are usable for testing an existing patch.
    """
...
def supports_patch_training(model: torch.nn.Module) -> bool:
    ...
    Exported backends (onnx, engine, openvino, tflite, ...) round-trip through numpy inside DetectMultiBackend.forward,
    so they cannot be trained against.
    """
